...
//поэтому описшу одну функцию остальные работают какжу
//поиск по фамилии
...
//		смотрим длину массива
		lenght := len(temp)
//		сравниваем искомое значение с средним элементо
//		и взависимости от результата создаем новый масси либо из левой, либо из правой части
		if temp[int(lenght/2)].Fio > fio {
			temp = temp[:int(lenght/2)]
		} else {
			temp = temp[int(lenght/2):]
		}
//		когда остался один элемент присваеваем его временной переменной и указываем искомы он или нет
		if lenght == 1 {
...
//	смотри длину массива
	length = len(c.Csmrs)
//	ставик ключ на первый элемент
	element = 0
	for {
		flagToNextPart := true
...
//			помощь по командам
			...
			fmt.Println("fio строка : поиск по фамилии")
